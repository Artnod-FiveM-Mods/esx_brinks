...
AddEventHandler('esx_brinks:hasExitedMarker', function(zone)
  printDebug('hasEnteredMarker: ' .. zone.name)
  if zone.name == 'bank' then TriggerServerEvent('esx_brinks:stopSellRun') end
  if zone.name == 'northBank' then TriggerServerEvent('esx_brinks:stopWeeklyCollect') end
  if zone.name == 'unionDepository' then TriggerServerEvent('esx_brinks:stopWeeklyDestruct') end
  currentAction    = nil
  currentActionMsg = ''
      ...
      if IsControlJustReleased(1, Keys["E"]) then
        ...
        elseif currentAction == 'harvestRun' then
          if not inVehicle then
            TriggerServerEvent('esx_brinks:harvestRun')
            nextMarket()
          else ESX.ShowNotification(_U('in_vehicle')) end
...
function openBrinksActionsMenu()
  ...
  printDebug(json.encode(playerData.job))
  local elements = {}  
      ...
      function(data, menu)
        if data.current.value ~= 'empty' then
          if data.current.type == 'item_standard' then
            ...
            printDebug('en dev')
            menu.close()
            ESX.TriggerServerCallback('esx_brinks:addArmoryWeapon', function() openPutStocksMenu() end, data.current.value, true)
          end
        end
      end,
      function(data, menu)
...
function vehicleMenu()
  local elements = {
    ...
    {label = Config.vehicles.bossCar.label, value = Config.vehicles.bossCar},
  }
...
function genMarketList()
  ...
  currentRun = newList
  printDebug('genMarketList: ' .. #Config.market - #currentRun .. '/' .. #Config.market)
end
function nextMarket()
  -- remove first line
  local tmpList = {}
...
function stopNativeJob()
  printDebug('stopNativeJob: ' .. #Config.market - #currentRun .. '1/' .. #Config.market)
  for i=1, #zoneList, 1 do
    if zoneList[i].name == 'market' then
